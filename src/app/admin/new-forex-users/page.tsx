...
export default function ForexUsersPage() {
  ...
  const [approveNewForexUser] = useApproveNewForexUserMutation();
  ...
  const handleAction = async () => {
    ...
    try {
      switch (type) {
        case "approve-user":
          toast.warning("⚠️ This will explicitly approve all the user's login ID, screenshots, and test trades.");
          await approveNewForexUser({ id: user._id }).unwrap();
          toast.success("User fully approved");
          break;
            ...
            <div
              className={`p-3 rounded-md mb-3 text-sm ${confirmAction.type === 'delete'
                ...
                : confirmAction.type.includes('reject')
                  ...
                  : confirmAction.type === 'approve-user'
                    ? 'bg-blue-100 text-blue-800 border border-blue-300'
                    : 'bg-muted-background-secondary text-muted-foreground border'
                }`}
            >
              ...
              {confirmAction.type === 'approve-user' && (
                <p>⚠️ This will approve all sections (Login ID, Account Screenshot, Test Trades) for this user.</p>
              )}
            </div>
          )}
