...
export function ChatDialog({
    ...
    useEffect(() => {
        ...
        // Construct WS URL - ensure we point to the root, not /api/api
        // Assuming NEXT_PUBLIC_API_URL includes /api, we might need to strip it or just use window.location.origin in prod
        // But standardized way usually replaces http with ws.
        // If NEXT_PUBLIC_API_URL is "http://localhost:3000/api", then wsUrl base is "ws://localhost:3000/api"
        // The backend path is "/afibe10x-chat", which usually means "ws://localhost:3000/afibe10x-chat" NOT inside /api

        // So we need to parse the origin.
        let baseUrl = "";
        try {
            const urlObj = new URL(apiUrl);
            baseUrl = `${urlObj.protocol === "https:" ? "wss:" : "ws:"}//${urlObj.host}`;
        } catch (e) {
            baseUrl = "ws://localhost:3000";
        }

        const wsUrl = `${baseUrl}/afibe10x-chat?adminId=${adminId}`;
